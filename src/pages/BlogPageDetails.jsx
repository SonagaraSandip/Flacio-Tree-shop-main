            ...
            <div className="flex flex-wrap gap-4 my-6">
              {BlogData[tab].map((post) =>
                post.tags?.map((tag, index) => (
                  <span
                    key={index}
                    className={`px-5 py-2 border border-gray-400 text-gray-500 font-poppins text-xs cursor-pointer hover:text-white hover:bg-gradient-to-r from-green-500 via-green-950 to-green-500 transition-all duration-800 hover:animate-moveStripes-slow hover:rounded`}
                  >
                    {tag}
                  </span>
                ))
